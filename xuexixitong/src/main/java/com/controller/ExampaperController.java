...
public class ExampaperController {
    ...
    */
    ...
    public R page(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        List<ExampaperEntity> exampaper = exampaperService.selectList(null);
        ArrayList<ExampaperEntity> exampaperEntities = new ArrayList<>();
        for (ExampaperEntity e:exampaper) {
            if(new Date().getTime() > e.getExampaperTime().getTime() && e.getExampaperTypes() == 1){
                ExampaperEntity exampaperEntity = new ExampaperEntity();
                exampaperEntity.setExampaperTypes(2);
                exampaperEntity.setId(e.getId());
                exampaperEntities.add(exampaperEntity);
            }
        }
        if(exampaperEntities != null && exampaperEntities.size() > 0){
            boolean b = exampaperService.updateBatchById(exampaperEntities);
            if(!b){
